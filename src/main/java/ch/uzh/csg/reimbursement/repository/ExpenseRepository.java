...
public interface ExpenseRepository extends JpaRepository<Expense, Integer> {
	...
	@Query("SELECT e FROM Expense e WHERE lower(e.accounting) LIKE lower(:accountingText) AND e.user IN :relevantUsers AND e.date >= :startTime AND e.date <= :endTime")
	public Set<Expense> search(@Param("relevantUsers") List<User> relevantUsers, @Param("accountingText") String accountingText, @Param("startTime") Date startTime, @Param("endTime") Date endTime);
